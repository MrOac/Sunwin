...
export default class Slot6Controller extends cc.Component {
  ...
  private showEffectJackpot(cash: number, cb: () => void = null) {
    ...
    this.effectJackpot
      .getComponentInChildren(sp.Skeleton)
      .setAnimation(0, animName[index], true);
    ...
    for (let i = 0; i < this.reels.childrenCount; i++) {
          ...
          if (i == 4) {
            ...
            for (let j = 0; j < this.columnsWild.length; j++) {
              ...
              cc.tween(this.iconWildColumns.children[c])
                .to(0.4, { scale: 1.6 }, { easing: "backOut" })
                .start();
              this.iconWildColumns.children[c].getComponent(
                sp.Skeleton
              ).animation = "animation";
              this.iconWildColumns.children[c].getComponent(sp.Skeleton).loop =
                true;
            }
      ...
      TW(roll)
        ...
        .call(() => {
          ...
          listItemNode[2]
            .getComponent(Slot6Item)
            .setId(parseInt(matrix[i]), true);
          listItemNode[1]
            .getComponent(Slot6Item)
            .setId(parseInt(matrix[5 + i]), true);
          listItemNode[0]
            .getComponent(Slot6Item)
            .setId(parseInt(matrix[10 + i]), true);
      ...
      switch (this.lastSpinRes.result) {
        ...
        case TYPE_WIN.BONUS: //bonus
          ...
          this.showEffectBonus(() => {
            if (this.soundSlotState == 1) {
              this.remoteMusicBackground = cc.audioEngine.playMusic(
                this.soundBgBonus,
                true
              );
            }
      ...
      BundleControl.loadPrefabGame(
        ...
        (prefab) => {
          ...
          this.popupJackpotHistory = cc
            .instantiate(prefab)
            .getComponent("Slot6.PopupJackpotHistory");

...
export default class PopupDetailHistory extends Dialog {
    ...
    show() {
        ...
        this.lblSession.string = App.instance.getTextLang('txt_session') + ":#" + this.session;
    ...
    private loadData() {
        ...
        Http.get(Configs.App.API, { "c": 102, "rid": this.session, "mt": Configs.App.MONEY_TYPE }, (err, res) => {
            ...
            if (res.success && res["resultTX"] !== null) {
                ...
                this.lblSession.string = App.instance.getTextLang('txt_session') + ":#" + res["resultTX"]["referenceId"];
                this.lblResult.string = res["resultTX"]["result"] == 1
                    ? " - " + App.instance.getTextLang('Tài') + " " + (res["resultTX"]["dice1"] + res["resultTX"]["dice2"] + res["resultTX"]["dice3"])  + "(" + res["resultTX"]["dice1"] + "-" + res["resultTX"]["dice2"] + "-" + res["resultTX"]["dice3"] + ")"
                    : " - " + App.instance.getTextLang('Xỉu') + " " + (res["resultTX"]["dice1"] + res["resultTX"]["dice2"] + res["resultTX"]["dice3"])  + "(" + res["resultTX"]["dice1"] + "-" + res["resultTX"]["dice2"] + "-" + res["resultTX"]["dice3"] + ")";
